...
def get_Top_Authors():
    ...
    """Define SQL-query and pass it over to the curser which execute the query and fetch the results"""
    sQuery = "select name ,sum from authors, ( select sum(count) as sum, author from (select slug, author, count from articles, pathCount where concat('/article/',cast(slug as varchar(20))) like concat('%', cast(path as varchar(20)) , '%')) as newtable group by author) as sumtable where authors.id = sumtable.author order by sum desc;"
    cCursor.execute(sQuery)
    results = cCursor.fetchall()
...
def count_Errors():
    ...
    """Define SQL-query and pass it over to the curser which execute the query and fetch the results"""
    sQuery = "select date, (oneperc*errorCount) as errorPercent from (select date(time) as date2, 100/cast(count(*) as decimal) as onePerc from log group by date2 order by date2) as onePercTable, (select date(time) , count(*) as errorCount from log where status != '200 OK' group by date order by date) as errorCountTable where onePercTable.date2 = errorCountTable.date and onePercTable.oneperc*errorcounttable.errorCount > 1;"
    cCursor.execute(sQuery)
    results = cCursor.fetchall()
    ...
    print (str(results[0][0]) + ': ' + str(round(results[0][1],2)) + '% Errors.\n')
